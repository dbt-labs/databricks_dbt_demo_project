{%- macro get_test_coverage() -%}
    ...
    {%- for test in enabled_tests -%}
        ...
        {%- for node in test.depends_on.nodes -%}
            ...
            {%- else -%}
                ...
                {%- do all_untested_nodes.append(node_name) -%}
    ...
    {%- set unique_untested_nodes = all_untested_nodes | unique | list  -%}

    {% do log("Project Test Coverage: " ~ pct ~ "%", info=True) %}
    {% do log("Untested models are:" ~unique_untested_nodes, info=True) %}
